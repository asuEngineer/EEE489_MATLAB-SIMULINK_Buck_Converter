...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) buck_converter_M->Timing.taskTime0;
  buck_converter_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(buck_converter_M->extModeInfo, &rtmGetTFinal
    (buck_converter_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(buck_converter_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(buck_converter_M);
